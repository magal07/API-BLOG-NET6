...
{
/*    #region Dependency Injection (TOKEN)
...
*/
    ...
    public async Task<IActionResult> Post([FromBody] RegisterViewModel model, [FromServices] BlogDataContext context)
    {
        ...
        {
            return StatusCode(400, new ResultViewModel<string>("DBEX400 - Email already exists"));
        }
        catch
        {
            return StatusCode(500, new ResultViewModel<string>("GENEX500 - Internal server error"));
        }
    }
    ...
    public async Task<IActionResult> Login([FromBody] LoginViewModel model, [FromServices] BlogDataContext context, [FromServices] TokenService tokenService)
    {
        ...
        if (user == null) return BadRequest(new ResultViewModel<string>(("Invalid Credentials")));

       if (!PasswordHasher.Verify(user.PasswordHash, model.Password))
       return StatusCode(401, new ResultViewModel<string>("Invalid Credentials"));
